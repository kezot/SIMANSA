...
<form method="post" action="<?php echo htmlspecialchars(Yii::app()->request->baseUrl . '/index.php?r=LihatStatistik'); ?>">
                ...
                <select name="selectMapel" >
                    ...
                    echo '<option ' . $selected1 . ' value="-1">Silahkan Pilih</option>';
                    //echo '<option>' . $selected . '</option>';
...
$listSiswa=array();
$arr=array();
if (isset($_POST['selectKelas']) && isset($_POST['selectMapel']) && $id_kelas > 0 && $id_mapel!='0') {
    $listSiswa = $this->getSiswa($id_kelas,$id_mapel);
    for ($index1 = 0; $index1 < count($listSiswa); $index1++) {
        //echo $listSiswa[$index1]->NIS.' ';
        $arr[$index1]=$listSiswa[$index1]->NILAI_KOGNITIF;
        //echo $arr[$index1].' ';
    }
}
...
// $listMapel=array();
// $arr1=array();
// if (isset($_POST['selectMapel']) && $id_mapel > 0) {
//     $listMapel = $this->getKelas($id_Mapel);
//     for ($index1 = 0; $index1 < count($listMapel); $index1++) {
//         //echo $listSiswa[$index1]->NIS.' ';
//         $arr1[$index1]=$listMapel[$index1]->KD_MATA_PELAJARAN_DIAJARKAN;
//         //echo $arr[$index1].' ';
//     }
// }
...
for ($index1 = 0; $index1 < count($result); $index1++) {
     $nilai[$index1]=$arr[$index1]->NILAI_KOGNITIF;
